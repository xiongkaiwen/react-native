...
export default class DistributeList extends Component {
	...
	_renderRow(data, sectionID, rowId){
		return(
			<TouchableOpacity key={data.id} activeOpacity={0.8} style={[styles.row,styles.j_center,styles.item]} onPress={()=>{
				this.props.navigator.push({
					component:Relationship,
					passProps:{sharerId:data.sharerId,shareRankId:this.props.shareRankId+1,_title:data.loginName},
				})
			}}>
				...
				<Text style={[styles.flex_1,styles._text]}>
					{this.renderRankName(data.shareRankId)}
				</Text>
				<Text style={[styles.flex_1,styles._text]}>
					{data.orderNum}
				</Text>
 ...
 // 获取数据
  getData(){
  	...
  	let postData = {
  		...
  		shareRankId:parseInt(this.props.shareRankId)+1,
  		sharerId:this.props.sharerId,
  	}
	Utils.post(Utils.domain+'app/sharers/pagequery',
				postData,
				(responData)=>{
  ...
  renderListViewHead(){
  	return (
  		<View style={[styles.row,styles.list_head]}>
  			...
  			<Text style={[styles.c14_333,styles.flex_1]}>级别</Text>
  			<Text style={[styles.c14_333,styles.flex_1]}>订单数</Text>
		</View>
  	);
  }
  ...
  renderHeader(){
	  return(<Header 
	  		initObj={{backName:'',title:this.props._title}} 
