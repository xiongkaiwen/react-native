...
// 找回支付密码
import FindPayPwd from './findpaypwd';
...
export default class PayPass extends Component {
  ...
  setUp(){
		...
		// 请求接口
		Utils.post(Utils.domain+'app/users/editpayPwd',
			{tokenId:global.tokenId,oldPass:orpayPwd,newPass:payPwd},
			function(responData){
  ...
  // 渲染头部
  renderHeader(){
	  return(<Header initObj={{backName:'',title: pwdType==1?'修改支付密码':'设置支付密码'}} navigator={this.props.navigator} />);
  }
	            ...
	            {this.primary()}
	         	...
	         	<TextInput keyboardType={'numeric'} style={styles.logininput} underlineColorAndroid={'transparent'} secureTextEntry={true} maxLength={6}  placeholder="确认密码" onChangeText={(val)=>this.setState({copayPwd:val})}/>
		         {
			         ...
			         <View style={styles.findpwd}>
		         		<Text onPress={this.findPayPwd}>忘记支付密码?</Text>
		         	</View>
		         	:
		         	null
		         }
